...
#Création d'un objet de type planet
class planet:
    ...
    #fonction qui change la position de la planète et la stocke dans un tableau qui est lui-même un attribut de la planète
...
#Créée des objets de type «planet» à des positions aléatoires
def spawn_planets(n_planets):

    all_planets = np.array([], dtype='object')

    # Les distributions des masses suivent une fonction Gaussienne
    mu=0.3
    sigma=0.4
...
def simulation(all_planets, speed=1, h=1/365, t_max=10):
    ...
    #Tableau de tous les masses des planètes pour pouvoir faire la grandeure du point qui représente la planète proportionnel à sa masse
    ...
    def animate(k):
        ...
        # Dessine tous les orbites en commenceant dans les derniers 100 pas de temps (dépendamment de «speed»)
...
# Premier argument est un tableaux d'objets de type planet, ces objets sont créés soit avec la fonction spawn_planets(n_planètes) qui les
